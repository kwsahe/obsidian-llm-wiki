...
- **카테고리 선택 화면**: 등록된 카테고리 카드 목록
- **비교 화면**: 상품 카드 나열(가격/판매처) + "우선순위 선택" 드롭다운 + "AI 비교 리포트 생성" 버튼
- **상품 상세 화면**: 가격 추이 라인 차트(Chart.js) + 매수 타이밍 코멘트 카드
- **비교 리포트 화면**: 기능별 승자 표 + LLM 생성 추천 문단

O2O-Demand-Forecasting-Solution의 대시보드 UI 패턴(KPI 카드, 차트, 랭킹 테이블)을 그대로 재사용하면 개발 속도를 크게 아낄 수 있다.
